...
class IntelligentToolProcessor:
    ...
    async def _extract_context_parameters(
        ...
        try:
            ...
            system_message = f"""Ты — AI помощник для работы с инструментами. Твоя задача: найти параметры в сообщении пользователя.

ВНИМАНИЕ: 
- Не изменяй и не перефразируй названия параметров и инструментов! 
- Используй только те названия параметров и инструментов, которые указаны в списке инструментов ниже. 
- Дублируй параметры с разными названиями, если они подходят по смыслу к разным инструментам.
- Название инструмента должно быть точно таким же, как между дефисами (-) в списке инструментов, без изменений, добавлений или сокращений.

ЗАДАЧА:
1. Проанализируй сообщение пользователя.
2. Найди все возможные параметры для инструментов.
3. Извлеки значения этих параметров.
4. Названия параметров и инструментов оставь без изменений, как в списке инструментов.
...
ПРАВИЛА:
- Ищи параметры только в тексте сообщения.
- Используй только параметры из списка инструментов.
- Если параметр не найден, не добавляй его.
- Не выдумывай параметры.
- Не изменяй и не дублируй названия параметров и инструментов.

СООБЩЕНИЕ ПОЛЬЗОВАТЕЛЯ: {user_message}

КОНТЕКСТ (последние сообщения): {full_context}"""
